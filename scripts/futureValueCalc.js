...
// dont run stuff until DOM is fully loaded
...
function calcFutureValue(event) {
    ...
    console.log(deposit, interestRate, years, "numbers from user");
    ...
    If you deposit <span class="fw-bold">$${deposit}</span> in a CD that earns ${interestRate}% interest and matures in
    ${years} years, your CD's ending balance will be $${futureValue.toFixed(2)} and you would have earned 
    $${interestEarned.toFixed(2)} in interest`
        ;
}

function resetForm() {
    alert("if I wasn't lazy this would have reset the form instead of these words")
}